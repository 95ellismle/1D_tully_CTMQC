...
class SingleSimData(object):
    ...
    conv_input_to_filename = {'|C|^2': ['ad pops'],
                 ...
                 'RI0': ['alt intercept'], 'sigmal': ['sigma_l'], 
                 'time': ['T'], 'u': ['diab coeff'], 'C': ['ad coeff'],
                 'Fad': ['ad force'], 'f': ['tot force'], 'NACV': ['dlk'],
    ...
    def _get_all_data(self, params_to_read):
        ...
        for param in params_to_read:
            poss_params = [key for key in self.conv_input_to_filename
                           if param.lower() in (j.lower() for j in self.conv_input_to_filename[key])
                           or 
                           param.lower() in key.lower()]
...
class NestedSimData(object):
    ...
    acess the data one can use the function `query_data`.
    """
    def __init__(self, folderpath, params_to_read=False):
        ...
        # Get all the data and store it in the flat dict
    ...
    def query_data(self, query_dict):
        ...
        return [self.__allData[i] for i in allInds]
...
class FredericaData(object):
    ...
    def find_high_low_mom(self, allMoms):
        """
        Will determine which momentum is high or low
        """
        allIsHigh = [int(i.split("_")[1].strip('K'))
                            for i in allMoms]
        allIsHigh = [i == np.max(allIsHigh) for i in allIsHigh]
        return allIsHigh
    
    def _read_all_data(self):
        """
        Will read Frederica's data.
        """
        ...
        for model in range(1, 4):
            ...
            allIsHigh = self.find_high_low_mom(os.listdir(newFolderO))
...
class GosselData(FredericaData):
    ...
    def find_high_low_mom(self, allMoms):
        ...
        Will determine which momentum is high or low
        """
        return ['high' in i.lower() for i in allMoms]
